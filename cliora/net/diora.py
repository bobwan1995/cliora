...
class DioraBase(nn.Module):
    ...
    def forward(self, x_span, x_word, obj_embed_span=None, obj_embed_word=None):
        ...
        # h: normalized word embeds, c: zero vec with the same shape
        h, c = self.leaf_transform(x_span)
        ...
        #     self.vg_atten_score = self.vg_atten_score_word
        #
        # self.atten_score = torch.diagonal(self.vg_atten_score, 0, 0, 1).permute(2, 0, 1) # TODO
